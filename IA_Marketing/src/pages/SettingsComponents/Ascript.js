...
const DEFAULT_CONTAINER_BACKGROUND_COLOR = "#ffffff"; // "#d3d3d3";
const DEFAULT_BTN_UPLOAD_BACKGROUND_COLOR = "#F7A400";
const COMPLETE_CONTAINER_BACKGROUND_COLOR = "#ffffff"; // "#d3d3d3";
...
const PAGE_REDIRECT = 'https://www.youtube.com/';
...
function uploadProcess() {
	...
	tl.to(uploadProgress, PROGRESS_TIME, {
		...
		ease: "power4.inOut",
		onStart: function () {
			// Tilt the progress "speech bubble" - inclinación bocadillo
		...
		onUpdate: function () {
			...
			// Increase the value of the progress (blue colored progress on progress bar) - naranja
	...
	tl.to(check2, {
		...
		onComplete: function () {
			setTimeout( () =>{
				...
				initialize();
			}, 3000);  //tiempo check final en miiseg
...
function initialize() {
	...
	btnUpload.style.opacity="0";
	// Redireccionar a otra pagina, otro componente, un modal, datos, otra función, etc...
	// const anklePageTest = `<a href="${PAGE_REDIRECT}">PAGE</a>`;
	// const anklePage = document.createElement("a");
	// anklePage.textContent = 'PAGE REDIRECT';
	// anklePage.href=PAGE_REDIRECT;
	// anklePage.className="page-redirect";
	window.location= PAGE_REDIRECT;

	container.appendChild(anklePage);
}
